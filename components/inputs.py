...
def render_input_panel():
    with st.container(border=True):
        ...
        with col1:
            ...
            temp = st.number_input("temp", min_value=50.0, max_value=350.0, value=float(st.session_state.inputs['temp']), step=5.0, format="%g", label_visibility="collapsed")
            
            st.markdown(custom_label("Sol. GOR at Pb", "scf/STB"), unsafe_allow_html=True)
            rs_pb = st.number_input("rs_pb", min_value=10.0, max_value=3000.0, value=float(st.session_state.inputs['rs_pb']), step=10.0, format="%g", label_visibility="collapsed")
        ...
        with col2:
            ...
            pb = st.number_input("pb", min_value=100.0, max_value=8000.0, value=float(st.session_state.inputs['pb']), step=50.0, format="%g", label_visibility="collapsed")
            
            st.markdown(custom_label("Producing GOR", "scf/STB"), unsafe_allow_html=True)
            gor = st.number_input("gor", min_value=0.0, max_value=100000.0, value=float(st.session_state.inputs['gor']), step=50.0, format="%g", label_visibility="collapsed")
    ...
    with st.container(border=True):
        ...
        <div style='display: flex; flex-direction: column; gap: 8px;'>
            ...
            <div style='display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: {THEME['bg']}; border-radius: 6px; border: 1px solid {THEME['border']};'>
                ...
                <span style='font-size: 12px; color: {THEME['text']}; font-weight: 500;'>Lee-Gonzalez-Eakin</span>
            </div>
            <div style='display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: {THEME['bg']}; border-radius: 6px; border: 1px solid {THEME['border']};'>
            ...
            <div style='display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: {THEME['bg']}; border-radius: 6px; border: 1px solid {THEME['border']};'>
                <span style='font-size: 12px; color: {THEME['muted']};'>Pseudo-Critical</span>
                <span style='font-size: 12px; color: {THEME['text']}; font-weight: 500;'>Standing</span>
            </div>
            <div style='display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: {THEME['bg']}; border-radius: 6px; border: 1px solid {THEME['border']};'>
                <span style='font-size: 12px; color: {THEME['muted']};'>Gas FVF</span>
                <span style='font-size: 12px; color: {THEME['text']}; font-weight: 500;'>Real Gas Law</span>
            </div>
        </div>
